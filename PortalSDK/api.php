...
class APIRequest {
	...
	function __get($ch) {
		...
		$response = wp_remote_get($this->context->get_url(), array('headers' => array('Authorization' => 'Bearer ' .$this->create_bearer_token(), 
		'Content-Type' => 'application/json','Host' => $this->context->get_address(),'Origin' => '*')));
		return new APIResponse(wp_remote_retrieve_response_code($response), "", $response['body']);
	}
	
	// Do a POST request
	function __post($ch) {
		$request_body = json_encode($this->context->get_parameters());
		$response = wp_remote_post($this->context->get_url(), array('headers' => array('Authorization' => 'Bearer ' .$this->create_bearer_token(), 
		'Content-Type' => 'application/json','Host' => $this->context->get_address(),'Origin' => '*'),'body' => $request_body));
		return new APIResponse(wp_remote_retrieve_response_code($response), "", $response['body']);
	}
...
class APIContext {
			...
			foreach($dictionary as $i => $item) {
				switch (strtolower($i)) {
					...
					case 'ssl':
						...
						break;
					case 'endpoint':
						$this->set_endpoint($dictionary[$i]);
						break;
					case 'method_type':
					...
					if (gettype($dictionary[$i]) != 'array') {
						...
						break;
					default:
						echo wp_kses_post('Unknown parameter type');
				}
			}
		}
	}
	...
	//Additional Getter and Setter for endpoint
